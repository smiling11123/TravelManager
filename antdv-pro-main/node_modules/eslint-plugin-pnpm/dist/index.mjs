...
import { dirname, resolve, basename, normalize, join } from 'pathe';
...
const enforceSettings = createEslintRule({
  name: RULE_NAME$3,
  meta: {
    type: "problem",
    fixable: "code",
  ...
  create(context, [options = {}]) {
    ...
    for (const [catalog, object] of Object.entries(catalogs)) {
      ...
      for (const [key, version] of Object.entries(object)) {
        ...
        if (existing) {
          const isDuplicate = checkDuplicates === "name-only" ? true : existing.version === version;
          if (isDuplicate) {
            const node = doc.getIn(catalog === "default" ? json.catalog ? ["catalog", key] : ["catalogs", catalog, key] : ["catalogs", catalog, key], true);
...
const validPackages = createEslintRule({
  name: RULE_NAME,
  meta: {
    type: "problem",
    docs: {
  ...
  create(context) {
    ...
    if (!parsed.packages || !Array.isArray(parsed.packages))
      return {};
    const doc = workspace.getDocument();
    const packagesNode = doc.getIn(["packages"]);
    if (!packagesNode)
      return {};
    const root = resolve(dirname(context.filename));
