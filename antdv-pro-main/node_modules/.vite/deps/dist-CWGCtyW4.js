import { ContextTracker, EditorSelection, EditorView, ExternalTokenizer, LRLanguage, LRParser, LanguageSupport, bracketMatchingHandle, foldNodeProp, indentNodeProp, parseMixed, styleTags, syntaxTree, tags } from "./dist-BSjRk5ij.js";
import { css, cssLanguage } from "./dist-Dh21ZLBs.js";
import { javascript, javascriptLanguage, jsxLanguage, tsxLanguage, typescriptLanguage } from "./dist-B6-BhQV2.js";
...
function maybeNest(node, input, tags$1) {
	let attrs;
	for (let tag of tags$1) if (!tag.attrs || tag.attrs(attrs || (attrs = getAttrs(node.node.parent.firstChild, input)))) return {
		parser: tag.parser,
		bracketed: true
	};
	return null;
}
function configureNesting(tags$1 = [], attributes = []) {
	let script = [], style = [], textarea = [], other = [];
	for (let tag of tags$1) (tag.tag == "script" ? script : tag.tag == "style" ? style : tag.tag == "textarea" ? textarea : other).push(tag);
	...
	return parseMixed((node, input) => {
		...
		if (id == Element && other.length) {
			let n = node.node, open = n.firstChild, tagName = open && findTagName(open, input), attrs$1;
			if (tagName) {
				for (let tag of other) if (tag.tag == tagName && (!tag.attrs || tag.attrs(attrs$1 || (attrs$1 = getAttrs(open, input))))) {
	...
	let closeTags = state.changeByRange((range) => {
		...
		if (didType && text == ">" && after.name == "EndTag") {
			...
			if (((_b = (_a = tag.parent) === null || _a === void 0 ? void 0 : _a.lastChild) === null || _b === void 0 ? void 0 : _b.name) != "CloseTag" && (name = elementName(state.doc, tag.parent, head)) && !selfClosers.has(name)) {
				let to$1 = head + (state.doc.sliceString(head, head + 1) === ">" ? 1 : 0);
				let insert = `</${name}>`;
				return {
					range,
					changes: {
...
//# sourceMappingURL=dist-CWGCtyW4.js.map