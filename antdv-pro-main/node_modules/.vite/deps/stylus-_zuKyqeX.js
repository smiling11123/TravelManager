...
function wordRegexp(words) {
	...
	return /* @__PURE__ */ new RegExp("^((" + words.join(")|(") + "))\\b");
}
...
function startOfLine(stream) {
	return stream.sol() || stream.string.match(/* @__PURE__ */ new RegExp("^\\s*" + escapeRegExp(stream.current())));
}
...
states.block = function(type$1, stream, state) {
	...
	if (type$1 == "word") {
		...
		if (override == "tag") {
			...
			if (stream.string.match(/* @__PURE__ */ new RegExp("\\[\\s*" + word + "|" + word + "\\s*\\]"))) {
				override = "atom";
				return "block";
			}
...
states.parens = function(type$1, stream, state) {
	...
	if (type$1 && type$1.charAt(0) == "@" && wordIsProperty(stream.current().slice(1))) override = "variable";
	if (type$1 == "word") {
		var word = stream.current();
		override = wordAsValue(word);
...
states.keyframes = function(type$1, stream, state) {
	...
	if (type$1 == "}") if (startOfLine(stream)) return popContext(state, stream, true);
...
states.interpolation = function(type$1, stream, state) {
	if (type$1 == "{") popContext(state, stream) && pushContext(state, stream, "block");
	if (type$1 == "}") {
...
states.extend = function(type$1, stream, state) {
	...
	if (type$1 == "]") return popContext(state, stream);
	if (type$1 == "word") {
		override = wordAsValue(stream.current());
		return "extend";
	}
...
const stylus = {
	...
	indent: function(state, textAfter, iCx) {
		var cx = state.context, ch$1 = textAfter && textAfter.charAt(0), indent = cx.indent, lineFirstWord = firstWordOfLine(textAfter), lineIndent = cx.line.indent, prevLineFirstWord = state.context.prev ? state.context.prev.line.firstWord : "", prevLineIndent = state.context.prev ? state.context.prev.line.indent : lineIndent;
		if (cx.prev && (ch$1 == "}" && (cx.type == "block" || cx.type == "atBlock" || cx.type == "keyframes") || ch$1 == ")" && (cx.type == "parens" || cx.type == "atBlock_parens") || ch$1 == "{" && cx.type == "at")) indent = cx.indent - iCx.unit;
		else if (!/(\})/.test(ch$1)) {
...
//#endregion
export { stylus };
//# sourceMappingURL=stylus-_zuKyqeX.js.map