import { ContextTracker, EditorSelection, EditorView, ExternalTokenizer, IterMode, LRLanguage, LRParser, LanguageSupport, LocalTokenGroup, NodeWeakMap, continuedIndent, defineLanguageFacet, delimitedIndent, flatIndent, foldInside, foldNodeProp, indentNodeProp, styleTags, sublanguageProp, syntaxTree, tags } from "./dist-BSjRk5ij.js";
import { completeFromList, ifNotIn, snippetCompletion } from "./dist-C4Wus0z5.js";
...
function getScope(doc, node) {
	...
	node.cursor(IterMode.IncludeAnonymous).iterate((node$1) => {
		if (top) top = false;
			...
			for (let c of getScope(doc, node$1.node)) completions.push(c);
			return false;
		}
	});
...
var autoCloseTags = /* @__PURE__ */ EditorView.inputHandler.of((view, from, to, text, defaultInsert) => {
	...
	let closeTags = state.changeByRange((range) => {
		...
		if (state.doc.sliceString(head - 1, head) != text || around.name == "JSXAttributeValue" && around.to > head);
			...
			let empty = around.parent, base$1 = empty.parent;
			if (base$1 && empty.from == head - 2 && ((name = elementName(state.doc, base$1.firstChild, head)) || ((_a = base$1.firstChild) === null || _a === void 0 ? void 0 : _a.name) == "JSXFragmentTag")) {
				let insert = `${name}>`;
				return {
...
//# sourceMappingURL=dist-B6-BhQV2.js.map