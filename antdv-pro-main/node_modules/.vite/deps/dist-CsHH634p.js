import { ContextTracker, ExternalTokenizer, IterMode, LRLanguage, LRParser, LanguageSupport, NodeWeakMap, delimitedIndent, foldInside, foldNodeProp, indentNodeProp, styleTags, syntaxTree, tags } from "./dist-BSjRk5ij.js";
...
function Context(parent, indent$1, flags) {
	this.parent = parent;
	this.indent = indent$1;
	this.flags = flags;
	this.hash = (parent ? parent.hash + parent.hash << 8 : 0) + indent$1 + (indent$1 << 4) + flags + (flags << 6);
}
var topIndent = new Context(null, 0, 0);
function countIndent(space$1) {
	let depth = 0;
	for (let i = 0; i < space$1.length; i++) depth += space$1.charCodeAt(i) == tab ? 8 - depth % 8 : 1;
	return depth;
}
...
function getScope(doc, node) {
	...
	node.cursor(IterMode.IncludeAnonymous).iterate((node$1) => {
		if (node$1.name) {
			...
			if (gather && gather(node$1, def, top) || !top && ScopeNodes.has(node$1.name)) return false;
			top = false;
...
//#endregion
export { python };
//# sourceMappingURL=dist-CsHH634p.js.map