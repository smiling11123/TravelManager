import { Fragment, Text, computed, createVNode, defineComponent, effectScope, getCurrentInstance, h, inject, isRef, onBeforeMount, onMounted, onUnmounted, ref, shallowRef, watch } from "./vue.runtime.esm-bundler-CYunRHQt.js";

//#region node_modules/vue-i18n/dist/vue-i18n.esm-browser.js
...
function createCompileWarn(code$4, loc, ...args) {
	const msg = format$1(warnMessages$2[code$4] || "", ...args || []);
	const message = {
		message: String(msg),
		code: code$4
	};
...
function createCompileError(code$4, loc, options = {}) {
	const { domain, messages, args } = options;
	const msg = format$1((messages || errorMessages$2)[code$4] || "", ...args || []);
...
function createTokenizer(source, options = {}) {
	...
	function emitError(code$4, pos, offset, ...args) {
		const ctx = context();
		pos.column += offset;
		pos.offset += offset;
		if (onError) {
			const loc = location ? createLocation(ctx.startLoc, pos) : null;
			const err = createCompileError(code$4, loc, {
...
function createParser(options = {}) {
	...
	function emitError(tokenzer, code$4, start, offset, ...args) {
	...
	function emitWarn(tokenzer, code$4, start, offset, ...args) {
		const end = tokenzer.currentPosition();
		end.offset += offset;
		end.column += offset;
		if (onWarn) {
			const loc = location ? createLocation(start, end) : null;
			onWarn(createCompileWarn(code$4, loc, args));
		}
	}
...
function createCodeGenerator(ast, options) {
	...
	const context = () => _context;
	function push(code$4, node) {
		_context.code += code$4;
	}
...
var generate = (ast, options = {}) => {
	...
	generator.push(`}`);
	delete ast.helpers;
	const { code: code$4, map } = generator.context();
	return {
		ast,
		code: code$4,
...
var code$3 = CompileWarnCodes.__EXTEND_POINT__;
var inc$3 = incrementer(code$3);
var CoreWarnCodes = {
	NOT_FOUND_KEY: code$3,
	FALLBACK_TO_TRANSLATE: inc$3(),
	CANNOT_FORMAT_NUMBER: inc$3(),
	FALLBACK_TO_NUMBER_FORMAT: inc$3(),
	CANNOT_FORMAT_DATE: inc$3(),
	FALLBACK_TO_DATE_FORMAT: inc$3(),
	EXPERIMENTAL_CUSTOM_MESSAGE_COMPILER: inc$3(),
...
function getWarnMessage$1(code$4, ...args) {
	return format$1(warnMessages$1[code$4], ...args);
}
...
function createCoreError(code$4) {
	return createCompileError(code$4, null, { messages: errorMessages$1 });
}
/** @internal */
var errorMessages$1 = {
...
function getWarnMessage(code$4, ...args) {
	return format$1(warnMessages[code$4], ...args);
}
...
function createI18nError(code$4, ...args) {
	return createCompileError(code$4, null, {
...
function updateComponentTreeTags(instance, treeNode, i18n) {
	const global$2 = i18n.mode === "composition" ? i18n.global : i18n.global.__composer;
	if (instance && instance.vnode.el && instance.vnode.el.__VUE_I18N__) {
		if (instance.vnode.el.__VUE_I18N__ !== global$2) {
			const tag = {
...
function useI18nForLegacy(instance, scope, root, options = {}) {
	...
	function setDateTimeFormat(locale$1, format$2) {
		if (_composer.value) {
			_composer.value.setDateTimeFormat(locale$1, format$2);
			_datetimeFormats.value[locale$1] = format$2;
		}
	}
	function mergeDateTimeFormat(locale$1, format$2) {
		if (_composer.value) _composer.value.mergeDateTimeFormat(locale$1, format$2);
	}
	function getNumberFormat(locale$1) {
	...
	function setNumberFormat(locale$1, format$2) {
		if (_composer.value) {
			_composer.value.setNumberFormat(locale$1, format$2);
			_numberFormats.value[locale$1] = format$2;
		}
	}
	function mergeNumberFormat(locale$1, format$2) {
		if (_composer.value) _composer.value.mergeNumberFormat(locale$1, format$2);
	}
	const wrapper = {
		get id() {
