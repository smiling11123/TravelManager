import { EditorSelection, EditorView, ExternalTokenizer, LRLanguage, LRParser, LanguageSupport, delimitedIndent, foldNodeProp, indentNodeProp, parseMixed, styleTags, syntaxTree, tags } from "./dist-BSjRk5ij.js";
import "./dist-C4Wus0z5.js";
import { html } from "./dist-CWGCtyW4.js";
	...
	for (;;) {
		...
		if (next == 123) {
			...
			if (after == 123) {
				...
				let scan = 2, size = 2;
				for (;;) {
					let next$1 = input.peek(scan);
					if (next$1 == 32 || next$1 == 10) ++scan;
					else if (next$1 == 35) {
						++scan;
						for (;;) {
							let comment$1 = input.peek(scan);
							if (comment$1 < 0 || comment$1 == 10) break;
							scan++;
						}
					} else if (next$1 == 45 && size == 2) size = ++scan;
					else {
						let end = next$1 == 101 && input.peek(scan + 1) == 110 && input.peek(scan + 2) == 100;
...
function rawTokenizer(endTag, text$1, tagStart$1) {
	return new ExternalTokenizer((input) => {
		...
		for (;;) {
			...
			if (next == 123 && input.peek(1) == 37) {
				...
				for (;; scan++) {
					let next$1 = input.peek(scan);
					if (!wordChar(next$1)) break;
					word += String.fromCharCode(next$1);
				}
				if (word == endTag) {
					if (input.pos > start) break;
					input.acceptToken(tagStart$1, 2);
					break;
				}
		...
		if (input.pos > start) input.acceptToken(text$1);
	});
}
...
function liquidCompletionSource(config = {}) {
	...
	let tags$1 = config.tags ? config.tags.concat(Tags) : Tags;
	...
	return (context) => {
		...
		else if (cx.type == "tag") options = tags$1;
...
cursor between them.
*/
var closePercentBrace = /* @__PURE__ */ EditorView.inputHandler.of((view, from, to, text$1) => {
	if (text$1 != "%" || from != to || view.state.doc.sliceString(from - 1, to + 1) != "{}") return false;
...
function liquid(config = {}) {
	let base$1 = config.base || baseHTML;
	let lang = base$1.language == baseHTML.language ? liquidLanguage : makeLiquid(base$1.language);
	return new LanguageSupport(lang, [
		base$1.support,
		lang.data.of({ autocomplete: liquidCompletionSource(config) }),
		base$1.language.data.of({ closeBrackets: { brackets: ["{"] } }),
		closePercentBrace
	]);
}
...
//#endregion
export { liquid };
//# sourceMappingURL=dist-DTcQ_Dgl.js.map