import { EditorSelection, EditorState, EditorView, Language, LanguageDescription, LanguageSupport, NodeProp, NodeSet, NodeType, ParseContext, Parser, Prec, Tag, Tree, countColumn, defineLanguageFacet, foldNodeProp, foldService, indentNodeProp, indentUnit, keymap, languageDataProp, parseMixed, styleTags, syntaxTree, tags } from "./dist-BSjRk5ij.js";
import { CompletionContext } from "./dist-C4Wus0z5.js";
import { html, htmlCompletionSource } from "./dist-CWGCtyW4.js";
...
var Type;
(function(Type$1) {
	Type$1[Type$1["Document"] = 1] = "Document";
	Type$1[Type$1["CodeBlock"] = 2] = "CodeBlock";
	Type$1[Type$1["FencedCode"] = 3] = "FencedCode";
	Type$1[Type$1["Blockquote"] = 4] = "Blockquote";
	Type$1[Type$1["HorizontalRule"] = 5] = "HorizontalRule";
	Type$1[Type$1["BulletList"] = 6] = "BulletList";
	Type$1[Type$1["OrderedList"] = 7] = "OrderedList";
	Type$1[Type$1["ListItem"] = 8] = "ListItem";
	Type$1[Type$1["ATXHeading1"] = 9] = "ATXHeading1";
	Type$1[Type$1["ATXHeading2"] = 10] = "ATXHeading2";
	Type$1[Type$1["ATXHeading3"] = 11] = "ATXHeading3";
	Type$1[Type$1["ATXHeading4"] = 12] = "ATXHeading4";
	Type$1[Type$1["ATXHeading5"] = 13] = "ATXHeading5";
	Type$1[Type$1["ATXHeading6"] = 14] = "ATXHeading6";
	Type$1[Type$1["SetextHeading1"] = 15] = "SetextHeading1";
	Type$1[Type$1["SetextHeading2"] = 16] = "SetextHeading2";
	Type$1[Type$1["HTMLBlock"] = 17] = "HTMLBlock";
	Type$1[Type$1["LinkReference"] = 18] = "LinkReference";
	Type$1[Type$1["Paragraph"] = 19] = "Paragraph";
	Type$1[Type$1["CommentBlock"] = 20] = "CommentBlock";
	Type$1[Type$1["ProcessingInstructionBlock"] = 21] = "ProcessingInstructionBlock";
	Type$1[Type$1["Escape"] = 22] = "Escape";
	Type$1[Type$1["Entity"] = 23] = "Entity";
	Type$1[Type$1["HardBreak"] = 24] = "HardBreak";
	Type$1[Type$1["Emphasis"] = 25] = "Emphasis";
	Type$1[Type$1["StrongEmphasis"] = 26] = "StrongEmphasis";
	Type$1[Type$1["Link"] = 27] = "Link";
	Type$1[Type$1["Image"] = 28] = "Image";
	Type$1[Type$1["InlineCode"] = 29] = "InlineCode";
	Type$1[Type$1["HTMLTag"] = 30] = "HTMLTag";
	Type$1[Type$1["Comment"] = 31] = "Comment";
	Type$1[Type$1["ProcessingInstruction"] = 32] = "ProcessingInstruction";
	Type$1[Type$1["Autolink"] = 33] = "Autolink";
	Type$1[Type$1["HeaderMark"] = 34] = "HeaderMark";
	Type$1[Type$1["QuoteMark"] = 35] = "QuoteMark";
	Type$1[Type$1["ListMark"] = 36] = "ListMark";
	Type$1[Type$1["LinkMark"] = 37] = "LinkMark";
	Type$1[Type$1["EmphasisMark"] = 38] = "EmphasisMark";
	Type$1[Type$1["CodeMark"] = 39] = "CodeMark";
	Type$1[Type$1["CodeText"] = 40] = "CodeText";
	Type$1[Type$1["CodeInfo"] = 41] = "CodeInfo";
	Type$1[Type$1["LinkTitle"] = 42] = "LinkTitle";
	Type$1[Type$1["LinkLabel"] = 43] = "LinkLabel";
	Type$1[Type$1["URL"] = 44] = "URL";
})(Type || (Type = {}));
/**
...
var LinkReferenceParser = class {
	...
	nextStage(elt$1) {
		if (elt$1) {
			this.pos = elt$1.to - this.start;
			this.elts.push(elt$1);
			this.stage++;
			return true;
		}
		if (elt$1 === false) this.stage = -1;
		return false;
	}
	advance(content) {
...
var BlockContext = class {
	...
	*/
	...
	advance() {
		...
		for (let parse of this.parser.leafBlockParsers) if (parse) {
			let parser$1 = parse(this, leaf);
			if (parser$1) leaf.parsers.push(parser$1);
		}
		lines: while (this.nextLine()) {
			...
			for (let parser$1 of leaf.parsers) if (parser$1.nextLine(this, line, leaf)) return null;
			leaf.content += "\n" + line.scrub();
	...
	*/
	addElement(elt$1) {
		this.block.addChild(elt$1.toTree(this.parser.nodeSet), elt$1.from - this.block.from);
	}
	/**
	...
	*/
	addLeafElement(leaf, elt$1) {
		this.addNode(this.buffer.writeElements(injectMarks(elt$1.children, leaf.marks), -elt$1.from).finish(elt$1.type, elt$1.to - elt$1.from), elt$1.from);
	}
	/**
	@internal
	*/
	...
	*/
	finishLeaf(leaf) {
		for (let parser$1 of leaf.parsers) if (parser$1.finish(this, leaf)) return;
	...
	*/
	configure(spec) {
		...
		if (nonEmpty(config.defineNodes)) {
			...
			let nodeTypes$1 = nodeSet.types.slice(), styles;
			for (let s of config.defineNodes) {
				let { name, block, composite, style } = typeof s == "string" ? { name: s } : s;
				if (nodeTypes$1.some((t) => t.name == name)) continue;
				if (composite) skipContextMarkup[nodeTypes$1.length] = (bl, cx, line) => composite(cx, line, bl.value);
				let id = nodeTypes$1.length;
				let group = composite ? ["Block", "BlockContext"] : !block ? void 0 : id >= Type.ATXHeading1 && id <= Type.SetextHeading2 ? [
					"Block",
					"LeafBlock",
					"Heading"
				] : ["Block", "LeafBlock"];
				nodeTypes$1.push(NodeType.define({
					id,
					name,
			...
			nodeSet = new NodeSet(nodeTypes$1);
		...
		if (nonEmpty(config.parseBlock)) for (let spec$1 of config.parseBlock) {
			let found = blockNames.indexOf(spec$1.name);
			if (found > -1) {
				blockParsers[found] = spec$1.parse;
				leafBlockParsers[found] = spec$1.leaf;
			} else {
				let pos = spec$1.before ? findName(blockNames, spec$1.before) : spec$1.after ? findName(blockNames, spec$1.after) + 1 : blockNames.length - 1;
				blockParsers.splice(pos, 0, spec$1.parse);
				leafBlockParsers.splice(pos, 0, spec$1.leaf);
				blockNames.splice(pos, 0, spec$1.name);
			}
			if (spec$1.endLeaf) endLeafBlock.push(spec$1.endLeaf);
		}
		if (nonEmpty(config.parseInline)) for (let spec$1 of config.parseInline) {
			let found = inlineNames.indexOf(spec$1.name);
			if (found > -1) inlineParsers[found] = spec$1.parse;
			else {
				let pos = spec$1.before ? findName(inlineNames, spec$1.before) : spec$1.after ? findName(inlineNames, spec$1.after) + 1 : inlineNames.length - 1;
				inlineParsers.splice(pos, 0, spec$1.parse);
				inlineNames.splice(pos, 0, spec$1.name);
			}
		}
...
var InlineContext = class {
	...
	@internal
	*/
	constructor(parser$1, text, offset) {
		this.parser = parser$1;
		this.text = text;
		this.offset = offset;
		/**
...
function parseCode(config) {
	...
	return { wrap: parseMixed((node, input) => {
		...
		if (codeParser && (id == Type.CodeBlock || id == Type.FencedCode)) {
			...
			let parser$1 = codeParser(info);
			if (parser$1) return {
				parser: parser$1,
				overlay: (node$1) => node$1.type.id == Type.CodeText,
...
function parseRow(cx, line, startI = 0, elts, offset = 0) {
	let count$1 = 0, first = true, cellStart = -1, cellEnd = -1, esc = false;
	let parseCell = () => {
...
function autolinkURLEnd(text, from) {
	...
	for (;;) {
		let last = text[end - 1], m$1;
		if (/[?!.,:*_~]/.test(last) || last == ")" && count(text, from, end, ")") > count(text, from, end, "(")) end--;
		else if (last == ";" && (m$1 = /&(?:#\d+|#x[a-f\d]+|\w+);$/.exec(text.slice(from, end)))) end = from + m$1.index;
		else break;
	}
	return end;
}
...
function parseSubSuper(ch, node, mark) {
	return (cx, next, pos) => {
		...
		for (let i = pos + 1; i < cx.end; i++) {
			let next$1 = cx.char(i);
			if (next$1 == ch) return cx.addElement(cx.elt(node, pos, i + 1, elts.concat(cx.elt(mark, i, i + 1))));
			if (next$1 == 92) elts.push(cx.elt("Escape", i, i++ + 2));
			if (space(next$1)) break;
		}
		return -1;
	};
}
...
function mkLang(parser$1) {
	return new Language(data, parser$1, [], "markdown");
}
/**
...
function getContext(node, doc) {
	...
	for (let i = nodes.length - 1; i >= 0; i--) {
		let node$1 = nodes[i], match;
		let line = doc.lineAt(node$1.from), startPos = node$1.from - line.from;
		if (node$1.name == "Blockquote" && (match = /^ *>( ?)/.exec(line.text.slice(startPos)))) context.push(new Context(node$1, startPos, startPos + match[0].length, "", match[1], ">", null));
		else if (node$1.name == "ListItem" && node$1.parent.name == "OrderedList" && (match = /^( *)\d+([.)])( *)/.exec(line.text.slice(startPos)))) {
			let after = match[3], len = match[0].length;
			if (after.length >= 4) {
				after = after.slice(0, after.length - 4);
				len -= 4;
			}
			context.push(new Context(node$1.parent, startPos, startPos + len, match[1], after, match[2], node$1));
		} else if (node$1.name == "ListItem" && node$1.parent.name == "BulletList" && (match = /^( *)([-+*])( {1,4}\[[ xX]\])?( +)/.exec(line.text.slice(startPos)))) {
			...
			context.push(new Context(node$1.parent, startPos, startPos + len, match[1], after, type, node$1));
		}
	}
	return context;
}
...
function normalizeIndent(content, state) {
	...
	return space$1 + content.slice(blank);
}
/**
	...
	let dont = null, changes = state.changeByRange((range) => {
		...
		if (inner.item && emptyLine) {
			...
			if (first.to >= pos || second && second.to < pos || line.from > 0 && !/[^\s>]/.test(doc.lineAt(line.from - 1).text) || config.nonTightLists === false) {
				...
				if (inner.node.name == "OrderedList") renumberList(inner.item, doc, changes$2, -2);
				if (next && next.node.name == "OrderedList") renumberList(next.item, doc, changes$2);
				return {
					range: EditorSelection.cursor(delTo + insert$1.length),
					changes: changes$2
				};
			} else {
				let insert$1 = blankLine(context, state, line);
				return {
					range: EditorSelection.cursor(pos + insert$1.length + 1),
					changes: {
						from: line.from,
						insert: insert$1 + state.lineBreak
					}
				};
			}
		}
		if (inner.node.name == "Blockquote" && emptyLine && line.from) {
			...
			if (quoted && quoted.index == inner.from) {
				...
				return {
					range: range.map(changes$2),
					changes: changes$2
				};
			}
		}
		let changes$1 = [];
		if (inner.node.name == "OrderedList") renumberList(inner.item, doc, changes$1);
...
function markdown(config = {}) {
	let { codeLanguages, defaultCodeLanguage, addKeymap = true, base: { parser: parser$1 } = commonmarkLanguage, completeHTMLTags = true, pasteURLAsLink: pasteURL = true, htmlTagLanguage = htmlNoMatch } = config;
	if (!(parser$1 instanceof MarkdownParser)) throw new RangeError("Base parser provided to `markdown` should be a Markdown parser");
	...
	let lang = mkLang(parser$1.configure(extensions));
...
//# sourceMappingURL=dist-BFuXW3Vg.js.map