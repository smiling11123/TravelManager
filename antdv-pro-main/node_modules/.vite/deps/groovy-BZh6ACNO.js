...
function startString(quote, stream, state) {
	...
	function t(stream$1, state$1) {
		var escaped = false, next, end = !tripleQuoted;
		while ((next = stream$1.next()) != null) {
			if (next == quote && !escaped) {
				if (!tripleQuoted) break;
				if (stream$1.match(quote + quote)) {
					end = true;
					break;
				}
			}
			if (quote == "\"" && next == "$" && !escaped) {
				if (stream$1.eat("{")) {
					state$1.tokenize.push(tokenBaseUntilBrace());
					return "string";
				} else if (stream$1.match(/^\w/, false)) {
					state$1.tokenize.push(tokenVariableDeref);
					return "string";
				}
			}
			escaped = !escaped && next == "\\";
		}
		if (end) state$1.tokenize.pop();
		return "string";
	}
...
//#endregion
export { groovy };
//# sourceMappingURL=groovy-BZh6ACNO.js.map