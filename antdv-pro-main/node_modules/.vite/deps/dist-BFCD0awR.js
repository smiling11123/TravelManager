import { ContextTracker, EditorSelection, EditorView, ExternalTokenizer, LRLanguage, LRParser, LanguageSupport, bracketMatchingHandle, foldNodeProp, indentNodeProp, styleTags, syntaxTree, tags } from "./dist-BSjRk5ij.js";
...
function completeFromSchema(eltSpecs, attrSpecs) {
	...
	for (let s of eltSpecs) {
		...
		if (s.attributes) attrs = attrs.concat(s.attributes.map((s$1) => {
			if (typeof s$1 == "string") return allAttrs.find((a) => a.label == s$1) || {
				label: s$1,
				type: "property"
			};
			if (s$1.values) {
				if (attrVals == attrValues) attrVals = Object.create(attrVals);
				attrVals[s$1.name] = s$1.values.map(valueCompletion);
			}
			return attrCompletion(s$1);
		}));
...
var autoCloseTags = /* @__PURE__ */ EditorView.inputHandler.of((view, from, to, text, insertTransaction) => {
	...
	let closeTags = state.changeByRange((range) => {
		...
		if (didType && text == ">" && after.name == "EndTag") {
			...
			let base$1 = after.parent;
			if (after.from == head - 2 && ((_c = base$1.lastChild) === null || _c === void 0 ? void 0 : _c.name) != "CloseTag" && (name = elementName(state.doc, base$1, head))) {
				let to$1 = head + (state.doc.sliceString(head, head + 1) === ">" ? 1 : 0);
				let insert = `${name}>`;
				return {
...
//#endregion
export { xml };
//# sourceMappingURL=dist-BFCD0awR.js.map