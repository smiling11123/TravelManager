	...
	getIndent(cx) {
		...
		while (statePos < cx.pos) {
			...
			if (line$1.length) {
				...
				while (stream.pos < end - line$1.from) readToken$1(this.streamParser.token, stream, state);
...
var Parse$1 = class {
	...
	emitToken(id$1, from, to, offset) {
		...
		if (this.lang.streamParser.mergeTokens && size == 4 && last >= 0 && this.chunk[last] == id$1 && this.chunk[last + 2] == from) this.chunk[last + 2] = to;
		else this.chunk.push(id$1, from, to, size);
		return offset;
	}
	parseLine(context) {
	...
	*/
	storeNode(term, start, end, size = 4, mustSink = false) {
		if (term == 0 && (!this.stack.length || this.stack[this.stack.length - 1] < this.buffer.length + this.bufferBase)) {
			let cur = this, top$1 = this.buffer.length;
			if (top$1 == 0 && cur.parent) {
				top$1 = cur.bufferBase - cur.parent.bufferBase;
				cur = cur.parent;
			}
			if (top$1 > 0 && cur.buffer[top$1 - 4] == 0 && cur.buffer[top$1 - 1] > -1) {
				if (start == end) return;
				if (cur.buffer[top$1 - 2] >= start) {
					cur.buffer[top$1 - 2] = end;
					return;
				}
			}
		}
...
var FragmentCursor = class {
	constructor(fragments, nodeSet$1) {
		this.fragments = fragments;
		this.nodeSet = nodeSet$1;
		this.i = 0;
		this.fragment = null;
		this.safeFrom = -1;
		this.safeTo = -1;
...
var LRParser = class LRParser extends Parser {
	...
	*/
	configure(config) {
		...
		if (config.tokenizers) copy.tokenizers = this.tokenizers.map((t$1) => {
			let found = config.tokenizers.find((r) => r.from == t$1);
...
//#endregion
export { Annotation, ChangeDesc, ChangeSet, CharCategory, Compartment, ContextTracker, Decoration, Direction, EditorSelection, EditorState, EditorView, ExternalTokenizer, Facet, HighlightStyle, IndentContext, IterMode, LRLanguage, LRParser, Language, LanguageDescription, LanguageSupport, LocalTokenGroup, MapMode, NodeProp, NodeSet, NodeType, NodeWeakMap, ParseContext, Parser, Prec, RangeSetBuilder, RangeValue, StateEffect, StateField, StreamLanguage, Tag, Text, Transaction, Tree, ViewPlugin, WidgetType, bracketMatchingHandle, codePointAt, codePointSize, combineConfig, continuedIndent, countColumn, crelt, defineLanguageFacet, delimitedIndent, drawSelection, findClusterBreak, flatIndent, foldInside, foldNodeProp, foldService, fromCodePoint, getIndentUnit, getIndentation, getPanel, getTooltip, indentNodeProp, indentString, indentUnit, keymap, languageDataProp, logException, matchBrackets, parseMixed, placeholder, runScopeHandlers, showPanel, showTooltip, styleTags, sublanguageProp, syntaxHighlighting, syntaxTree, tags };
//# sourceMappingURL=dist-BSjRk5ij.js.map