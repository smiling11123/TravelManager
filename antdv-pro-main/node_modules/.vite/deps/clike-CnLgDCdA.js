...
function words(str) {
	var obj = {}, words$1 = str.split(" ");
	for (var i = 0; i < words$1.length; ++i) obj[words$1[i]] = true;
	return obj;
}
function contains(words$1, word) {
	if (typeof words$1 === "function") return words$1(word);
	else return words$1.propertyIsEnumerable(word);
}
...
function tokenRawString(stream, state) {
	...
	if (stream.match(/* @__PURE__ */ new RegExp(".*?\\)" + delim + "\""))) state.tokenize = null;
...
function tokenDartString(quote, stream, state, raw) {
	...
	function tokenStringHelper(stream$1, state$1) {
		var escaped = false;
		while (!stream$1.eol()) {
			if (!raw && !escaped && stream$1.peek() == "$") {
				pushInterpolationStack(state$1);
				state$1.tokenize = tokenInterpolation;
				return "string";
			}
			var next = stream$1.next();
			if (next == quote && !escaped && (!tripleQuoted || stream$1.match(quote + quote))) {
				state$1.tokenize = null;
				break;
			}
...
//# sourceMappingURL=clike-CnLgDCdA.js.map