import { ExternalTokenizer, IterMode, LRLanguage, LRParser, LanguageSupport, LocalTokenGroup, NodeWeakMap, continuedIndent, foldInside, foldNodeProp, indentNodeProp, styleTags, syntaxTree, tags as tags$1 } from "./dist-BSjRk5ij.js";
...
var identifierTokens = (id, varName, callee$1) => (input, stack) => {
	for (let inside = false, dashes = 0, i = 0;; i++) {
		...
		if (isAlpha(next) || next == dash || next == underscore || inside && isDigit(next)) {
			...
			if (inside) input.acceptToken(dashes == 2 && stack.canShift(VariableName) ? varName : next == parenL ? callee$1 : id);
			break;
		}
	}
};
...
function isVarArg(node, doc) {
	...
	let callee$1 = (_a = node.parent) === null || _a === void 0 ? void 0 : _a.firstChild;
	if ((callee$1 === null || callee$1 === void 0 ? void 0 : callee$1.name) != "Callee") return false;
	return doc.sliceString(callee$1.from, callee$1.to) == "var";
}
...
function variableNames(doc, node, isVariable) {
	if (node.to - node.from > 4096) {
		...
		node.cursor().iterate((node$1) => {
			var _a;
			if (isVariable(node$1) && node$1.matchContext(declSelector) && ((_a = node$1.node.nextSibling) === null || _a === void 0 ? void 0 : _a.name) == ":") {
				let name = doc.sliceString(node$1.from, node$1.to);
				if (!seen.has(name)) {
					seen.add(name);
					result.push({
						label: name,
...
//# sourceMappingURL=dist-Dh21ZLBs.js.map